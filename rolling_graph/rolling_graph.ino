...
void draw ()
{
  ...
  for (int s=0 ; s < CHANNELS ; ++s) {
    ...
    maxv = max(max(maxv, abs(maxArray(samples, WIDTH))), abs(minArray(samples, WIDTH)));
    if (ABSMAX != -1) {
  ...
  for (int s=0 ; s < CHANNELS ; ++s) {
    ...
    float[] samples = series[s];
    float seriesMax = maxArray(samples, WIDTH);
    
    text("ch " + s + " cur:" + samples[ptr] + " max:" + seriesMax + " min:" + minArray(samples, WIDTH), 0, 8 + 10 * s);
